...
1. **Scenario**: Cambio de estado exitoso y emisión del evento solicitar_ruta
   ...
   - **Then** el sistema registra el cambio en el historial con timestamp UTC e ID del almacenista, e invoca automáticamente el caso de uso `Solicitar Ruta de Paquete`, que construye y envía el evento `solicitar_ruta` al Módulo 2 con el payload completo.
...
3. **Scenario**: El Módulo 2 rechaza el evento solicitar_ruta — reversión de estado
...
- What happens when el Módulo 2 rechaza el evento `solicitar_ruta` por campo inválido? El sistema revierte el estado a `En Clasificación`, notifica al almacenista con el detalle del error y permite corregir el dato para reintentar la transición completa.
...
- **FR-006**: System MUST invocar automáticamente el caso de uso `Solicitar Ruta de Paquete` cuando el estado cambie a `Listo para Despacho`, el cual emitirá el evento `solicitar_ruta` al Módulo 2 con todos los campos requeridos en el Contrato de Integración v1.0.
- **FR-007**: System MUST revertir el estado a `En Clasificación` si el Módulo 2 rechaza el evento `solicitar_ruta`, notificando al almacenista con el detalle del campo inválido.
- **NFR-001**: System MUST asegurar que el tiempo promedio de actualización de estado no supere los 2 minutos por paquete.
- **NFR-002**: System MUST garantizar que la emisión del evento `solicitar_ruta` se realice de forma transaccional: o se actualiza el estado Y se emite el evento, o ninguna de las dos operaciones se persiste.
...
- **SC-003**: El 100% de las actualizaciones exitosas a `Listo para Despacho` deben disparar el evento `solicitar_ruta` al Módulo 2, o revertir el estado a `En Clasificación` si ocurre un fallo.