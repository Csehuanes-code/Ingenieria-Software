...
Como Empleado de Envío y Recepción, necesito registrar los datos completos de un paquete nuevo —incluyendo remitente, destinatario y dirección de destino— para generar su identidad única, capturar su peso de forma obligatoria y disparar automáticamente la solicitud de ruta oficial al Módulo 2, todo desde una sola operación.

**Why this priority**: Es el punto de entrada al sistema. Orquesta la captura de datos del cliente, el pesaje obligatorio y la solicitud de ruta. Sin este registro el paquete no existe para ningún otro módulo.

**Independent Test**: Puede probarse completando el formulario con datos válidos de remitente, destinatario y dirección, ejecutando el pesaje y confirmando el registro. El test es exitoso si el sistema genera un UUID, emite la etiqueta y se comprueba en los logs que el evento de solicitud de ruta fue enviado al Módulo 2.

**Acceptance Scenarios**:

1. **Scenario**: Registro exitoso con solicitud de ruta automática
   ...
   - **Then** el sistema genera un UUID único, registra la fecha/hora y sede automáticamente, actualiza el estado a `Recibido en Sede`, emite la etiqueta física e invoca automáticamente el caso de uso `Solicitar Ruta de Paquete` .
...
2. **Scenario**: Registro con fallo de geolocalización — estado Pendiente GPS
   ...
   - **Then** el sistema permite guardar la dirección en texto plano, asigna el estado `Pendiente GPS` al campo geográfico, bloquea la invocación del caso de uso `Solicitar Ruta de Paquete` hasta que las coordenadas sean resueltas, y notifica al empleado el estado parcial del registro.

---

### Edge Cases

- What happens when el servicio de geocodificación falla (timeout > 5 segundos)? El sistema muestra un aviso al empleado. Se permite ingresar coordenadas manualmente o guardar la dirección con estado `Pendiente GPS`. La solicitud de ruta al Módulo 2 queda en pausa hasta que las coordenadas sean resueltas correctamente.
- What happens when se intenta confirmar el registro sin haber completado el pesaje? El sistema bloquea la confirmación y muestra un mensaje indicando que el caso de uso `Procesar Pesaje y Dimensiones` es obligatorio antes de continuar.
...
- What happens when el empleado cancela el formulario después de que ya se generó un UUID preliminar? El registro queda en estado `Borrador` por 30 minutos y es eliminado automáticamente si no se confirma. No se emite ninguna solicitud de ruta.
...
- **FR-011**: System MUST invocar automáticamente el caso de uso `Solicitar Ruta de Paquete`  al finalizar exitosamente el registro y pesaje, para reservar espacio en el Módulo 2 desde el momento de la admisión.
...
- **SC-002**: El 100% de los paquetes con registro exitoso deben disparar la solicitud de ruta al Módulo 2 al finalizar la admisión.
- **SC-003**: El 95% de las direcciones registradas deben resolverse en coordenadas GPS válidas en tiempo real durante el registro.