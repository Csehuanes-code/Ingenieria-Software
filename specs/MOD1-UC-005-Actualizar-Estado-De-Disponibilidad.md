...
3. **Scenario**: El `Módulo de Gestión de Rutas` rechaza el evento [Solicitar Ruta de Paquete(MOD1-UC-003)](./MOD1-UC-003-Solicitar-Ruta-De-Paquete.md) — reversión de estado
   - **Given** el almacenista confirmó el cambio a `Listo para Despacho` y el evento fue enviado al `Módulo de Gestión de Rutas`.
   - **When** el `Módulo de Gestión de Rutas` responde con error detallando el campo inválido o ausente.
...
- What happens when el `Módulo de Gestión de Rutas` no responde al recibir el evento? El sistema reintenta un máximo de 3 veces con intervalos de 30 segundos. Si el fallo persiste, el paquete permanece en `Listo para Despacho` y se genera una alerta al despachador para gestión manual.
...
- **FR-006**: System MUST invocar automáticamente el caso de uso [Solicitar Ruta de Paquete(MOD1-UC-003)](./MOD1-UC-003-Solicitar-Ruta-De-Paquete.md) cuando el estado cambie a `Listo para Despacho`, el cual emitirá el evento [Solicitar Ruta de Paquete(MOD1-UC-003)](./MOD1-UC-003-Solicitar-Ruta-De-Paquete.md) al `Módulo de Gestión de Rutas` con todos los campos requeridos en el Contrato de Integración v1.0.
- **FR-007**: System MUST revertir el estado a `En Clasificación` si el `Módulo de Gestión de Rutas` rechaza el evento [Solicitar Ruta de Paquete(MOD1-UC-003)](./MOD1-UC-003-Solicitar-Ruta-De-Paquete.md), notificando al almacenista con el detalle del campo inválido.
...
- **SC-003**: El 100% de las actualizaciones exitosas a `Listo para Despacho` deben disparar el evento [Solicitar Ruta de Paquete(MOD1-UC-003)](./MOD1-UC-003-Solicitar-Ruta-De-Paquete.md) al `Módulo de Gestión de Rutas`, o revertir el estado a `En Clasificación` si ocurre un fallo.